...
// 📥 यहाँ Zap आइकॉन को भी पूरी तरह शामिल कर लिया गया है
...
export default function Sidebar({ currentPanel, setCurrentPanel, isOpen, setIsOpen }) {
  ...
  const menuItems = [
    ...
    { id: "stock-panel", text: "Live Stock Inventory", icon: Zap }, // आइकॉन पूरी तरह परिभाषित है
    { id: "register-user", text: "Register User", icon: UserPlus },
    { id: "manage-user", text: "Manage User", icon: Users },
  ...
  const handleNavClick = (id) => {
    setCurrentPanel(id);
    setIsOpen(false); // मोबाइल व्यू पर क्लिक करते ही साइडबार बंद होगा
  };

  return (
    <>
      {/* मोबाइल बैकड्रॉप स्क्रीन ब्लर इफ़ेक्ट */}
      ...
      {/* मुख्य साइडबार कंटेनर */}
      <div
        className={`w-64 h-screen bg-slate-900 text-white flex flex-col fixed left-0 top-0 z-40 shadow-2xl transition-transform duration-300 md:translate-x-0 ${
      ...
      >
            ...
            return (
              <button
                ...
                className={`w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all duration-150 ${
                  isActive ? "bg-blue-600 text-white shadow-md shadow-blue-600/20" : "text-slate-400 hover:bg-slate-800 hover:text-white"
                }`}
              >
        ...
        <div className="p-3 border-t border-slate-800">
          <button
            onClick={logout}
            className="w-full flex items-center justify-center space-x-2 py-2.5 bg-red-600/10 hover:bg-red-600 text-red-400 hover:text-white rounded-xl font-bold transition duration-150"
          >
